...
describe('generateSlug', () => {
  ...
  it('1만 개를 만들어도 겹치는 것이 거의 없다', () => {
    const slugs = new Set(Array.from({ length: 10_000 }, () => generateSlug()));

    // 생일 문제. 조합 수 N=10.7억에서 n=1만 개를 뽑으면 충돌 개수의 기댓값은
    // n²/2N ≈ 0.047 이다. 즉 스무 번 돌리면 한 번쯤 한 쌍이 겹친다.
    // "정확히 1만 개"로 검사하면 코드에 아무 문제가 없어도 5% 확률로 실패한다.
    // 충돌 6개 이상은 사실상 불가능하므로 그 선을 긋는다.
    // 실제로 겹쳐도 DB의 unique 제약이 막고 createTeam이 다시 만든다.
    expect(slugs.size).toBeGreaterThanOrEqual(9_995);
  });
